...
void check_trama(char *buffer)
{
    int i = 0;
    int size_buffer = sizeof(*buffer);
    if ((*buffer == '>') && (*(buffer + 1) == 'S') && (*(buffer + 2) == 'W') && (*(buffer + 3) == ':') && (*(buffer + 5) == ','))
    {
        flag_send = 1;
    }
}
...
int main(void)
{
    ...
    *connfd = accept(sockfd, (SA *)&cli, &len);
    ...
    // Function for chatting between cl1ient and server
    ...
    while (1)
    {
        ...
        if (flag_send == 0)
        {
            ...
            //  check_trama(serial_buff);
            serial_receive(serial_buff, buff_size);
